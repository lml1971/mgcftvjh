 ...
 *      ★ 后续需要修改的只有两处：「直播源」和「引流节目」
 *        ——分别在 [MODIFY-1] 与 [MODIFY-2] 标记。
 ...
 *   - ENABLE_PROMO      : "true" | "false"  是否注入引流节目，默认 true
...
const EXACT_GROUP_RULES = [
    // 引流节目：由 PROMO_LIST [MODIFY-2] 硬编码独立置顶
    { group: "推流信息", test: t => /^(幸福家|老李卡通|我们一家|25年前)$/.test(t) },
    // 咪咕独立分组
...
async function loadAllChannels(env, ctx) {
    ...
    if (enablePromo && PROMO_LIST.length > 0) {
        console.log(`[loadAllChannels] 引流节目注入已启用 (${PROMO_LIST.length} 条)`);
    } else {
        console.log(`[loadAllChannels] 引流节目注入已禁用`);
    }
    ...
    const writeKV = async () => {
        ...
        try {
            await env.KV.put(KV_CACHE_KEY, JSON.stringify({
                channels: deduped,
...
// 固定分组顺序：引流置顶，地方台拆为「XX地区」子分组排在春晚/熊猫之前
const MAIN_GROUP_ORDER = [
    "推流信息", "中央频道", "📺 高清频道", "📡 卫视", "🎬 影视", "📡 咪咕", "🏆 体育", "📰 新闻",
    "🎵 音乐人", "🎭 小品曲艺", "📺 港澳台",
    "👶 少儿", "📚 教育", "📺 其他频道",
...
if (typeof module !== "undefined" && module.exports) {
    module.exports = {
        ...
        buildM3U, buildTXT,
        loadAllChannels, mergeAndDedup, speedTest, probeUrl,
        MAIN_GROUP_ORDER,
    };
}
