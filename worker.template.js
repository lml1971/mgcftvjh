 ...
 *   3. 分组策略：频道名规范化 + 分组合并（央视/卫视/地方/香港/台湾/澳门）
 *      + 频道归类（其他频道按名称重新归类）+ URL去重 + 死链探测（可选）
 *      + 择优排序。输出时按 推流→央视→卫视→地方→港澳台→体育→影视→… 排序。
 ...
 *        PROBE_DEAD_LINKS    "true" 开启死链探测+择优排序（默认关闭）
 *        PROBE_TIMEOUT_MS   单URL探测超时毫秒（默认 5000）
 *        PROBE_MAX_URLS      最多探测URL数（默认 30）
...
const PROMO_LIST = [
    // { title: "宣传片", url: "https://...mp4", pic: "https://...jpg", group: "我的推流", from: "线路A", remarks: "置顶引流" },
];
